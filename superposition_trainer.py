...
def do_test(cfg, model, iteration=0):
    ...
    for dataset_name in cfg.DATASETS.TEST:
        ...
        if comm.is_main_process():
            ...
            csv_path = f"{cfg.OUTPUT_DIR}/{dataset_name}.csv"
            if not os.path.exists(csv_path):
                with open(csv_path, "a+") as f:
                    f.write("epoch,AP,AP50,AP75,APs,APm,APl\n")
            APs = results_i["bbox"]
            with open(csv_path, "a+") as f:
                ap_str = [str(iteration)] + [str(round(APs[key], 4)) for key in
...
def do_train(cfg, model, auxiliary_model=None, resume=False):
    ...
    if auxiliary_type == "Random":
        ...
        fixed_distribution_list = None
        if cfg.AUXILIARY.FIXED_DISTRIBUTION_LIST is not None:
            fixed_distribution_list = cfg.AUXILIARY.FIXED_DISTRIBUTION_LIST.split(",")
...
def auxiliary_setup(cfg, args):
    ...
    arg_name_list = ["feature_lambda", "superposition_start", "auxiliary_type", "random_layers", "progressive",
                     ...
                     "random_auxiliary_initial_way", "init_distribution_fixed", "p", "fixed_distribution_list"]
...
def auxiliary_argument_parser(parser):
    ...
    parser.add_argument('--fixed-distribution-list', type=str, default=None, help='the fixed distribution provided.')
    parser.add_argument('--superposition-way', type=str, choices=['MixStyle', 'Direct', 'None'],
                        default='None', help='the way of superposition for the auxiliary network')

...
def get_filename(file_path, need_suffix=False):
    if need_suffix:
        ...
        return os.path.basename(file_path).replace(f".{get_suffix(file_path)}", "")
...
def convert(xml_files, json_file, img_dir_path):
    ...
    for idx, xml_file in enumerate(xml_files):
        ...
        if len(path) == 1:
            ...
            try:
                ...
                filename = get_and_check(root, "name", 1).text
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--img_dir", help="Directory path to xml files.", type=str)
    ...
    convert(xml_files, args.json_file, args.img_dir)
